...
class Solution {
    public int solution(String[] want, int[] number, String[] discount) {
        ...
        for (int i = 0; i < discount.length - 9; i++) {
            ...
            // 10일 동안 할인하는 제품의 개수를 HashMap에 저장
            for (int j = i; j < 10 + i; j++) hm.put(discount[j], hm.getOrDefault(discount[j], 0) + 1);
            
            // 원하는 제품을 모두 할인 받을 수 있는지 없는지 확인
            for (int j = 0; j < want.length; j++) {
                if (hm.get(want[j]) == null || hm.get(want[j]) != number[j]) {
                    isSatisfied = false;
                    break;
                } 
            }
